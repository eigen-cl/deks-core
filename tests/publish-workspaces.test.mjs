...
test("declares the release versions and exact internal dependency closure", async () => {
  ...
  assert.equal(react.version, "0.4.0");
  assert.equal(preview.version, "0.2.2");
  assert.equal(renderer.dependencies["@deks-js/document"], "0.3.0");
  assert.deepEqual(react.dependencies, {
    "@deks-js/document": "0.3.0",
    "@deks-js/renderer-core": "0.5.0",
  });
  assert.equal(preview.dependencies["@deks-js/document"], "0.3.0");
  assert.equal(preview.dependencies["@deks-js/renderer-core"], "0.5.0");
  assert.equal(preview.dependencies.playwright, "1.62.1");

  for (const [path, version] of [
    ["", "0.5.0"],
    ["packages/document", "0.3.0"],
    ["packages/renderer-core", "0.5.0"],
    ["packages/react", "0.4.0"],
    ["packages/render-preview", "0.2.2"],
  ]) assert.equal(lock.packages[path].version, version);
  assert.equal(lock.packages["packages/react"].dependencies["@deks-js/renderer-core"], "0.5.0");
  assert.equal(lock.packages["packages/render-preview"].dependencies["@deks-js/renderer-core"], "0.5.0");
});
